#include<bits/stdc++.h>
#include <ext/pb_ds/assoc_container.hpp> 
#include <ext/pb_ds/tree_policy.hpp> 
#define ll long long int
#define MOD 1000000007
#define oo 1000000000000000000
#define forr(i,n) for(ll i=0;i<n;i++)
#define fastio ios_base::sync_with_stdio(false); cin.tie(0) ; cout.tie(0);
#define all(x) x.begin(),x.end()
#define eb emplace_back
#define pb push_back
#define ordered_set tree<int, null_type,less<int>, rb_tree_tag,tree_order_statistics_node_update> 
 
using namespace __gnu_pbds; 
using namespace std;

ll valueOfIndex(ordered_set&s , ll i){ return *(s.find_by_order(i)); }
ll indexOfValue(ordered_set&s , ll x){ return s.order_of_key(x); }
ll add(ll a, ll b,ll p=MOD) { a%=p; b%=p; return (a+b + p)%p;}
ll mul(ll a, ll b,ll p=MOD) { a%=p; b%=p; return (a*b + p)%p;} // __int128
ll power(ll x,ll n,ll p=MOD){ if(x==0) return 0; if(n==0 || x==1) return 1LL;
    ll r = (power(x,n/2,p))%p; if(n&1) return mul(mul(r,r,p) , x,p); else return mul(r,r,p);
}

class LCA{ // lca with sparsh table in O(1) time 
   private:
    vector<int> euler , height , logTable , first;
    vector<vector<int>> v;
   public:
    LCA(vector<vector<int>> &adj, int n , int root = 1){
        euler.clear(); euler.eb(-1);
        height.assign(n+2,0);
        first.assign(n+2,0);
        dfs(adj,root,-1);
        int sz = euler.size();
        logTable.assign(sz+5,0);
        logTable[1] = 0;
        for(int i=2;i<=sz+3;i++) logTable[i] = logTable[i/2]+1;
        v.clear(); v.assign(logTable[sz+2]+1 , vector<int>(sz+2,0));
        pre(sz-1);
    }
    void dfs(vector<vector<int>> &adj,int node, int par, int h=1){
        first[node] = euler.size();
        euler.eb(node);
        height[node] = h;
        for(auto &i: adj[node]){
            if(i!=par){
                dfs(adj,i,node,h+1);
                euler.eb(node);
            }
        }
        return;
    }
    void pre(int n){
        for(int i=1;i<=n;i++) v[0][i] = euler[i];
        for(int lg=1;lg<=logTable[n];lg++){
            for(int i=1;i+(1<<lg)-1<=n;i++){
                int p = v[lg-1][i] , q = v[lg-1][i + (1<<(lg-1))];
                if( height[p] < height[q] ) v[lg][i] = p;
                else v[lg][i] = q;
            }
        }
    }
    int lca(int a, int b){
        int l = first[a] , r = first[b];
        if(l > r) swap(l,r);
        int lg = logTable[r-l+1];
        int p = v[lg][l] , q = v[lg][r - (1<<lg) + 1];
        if(height[p] < height[q]) return p;
        else return q;
    }

};



void __sol(){
    
    vector<vector<int>> adj(8);
    adj[1].eb(2); adj[2].eb(1);
    adj[1].eb(3); adj[3].eb(1);
    adj[1].eb(4); adj[4].eb(1);
    adj[2].eb(5); adj[5].eb(2);
    adj[2].eb(6); adj[6].eb(2);
    adj[4].eb(7); adj[7].eb(4);

    LCA t(adj,7);
    cout << t.lca(2,7);
    
    return;
}


int main(){
    fastio
    ll tc=1; // cin >> tc;
    while(tc--) __sol();
    return 0;
}